...
def make_p(runs, align="left", space_before=20, space_after=20, line_spacing=220):
    ...
    for item in runs:
        if isinstance(item, str):
            t, b, i, sz, c = item, False, False, 26, "000000"
        else:
            t = item[0]
            b = item[1] if len(item) > 1 else False
            i = item[2] if len(item) > 2 else False
            sz = item[3] if len(item) > 3 else 26
...
def make_callout(text, title=None, bg="F0F4F8", bdr="1E3A8A"):
    p_pr = f'''<w:pPr>
        <w:pBdr><w:left w:val="single" w:sz="18" w:space="10" w:color="{bdr}"/></w:pBdr>
    ...
    runs_xml = []
    if title:
        runs_xml.append(f'''<w:r><w:rPr><w:rFonts w:ascii="TH Sarabun PSK" w:hAnsi="TH Sarabun PSK" w:cs="TH Sarabun PSK"/><w:b/><w:bCs/><w:sz w:val="26"/><w:szCs w:val="26"/><w:color w:val="{bdr}"/></w:rPr><w:t xml:space="preserve">{esc(title)}&#10;</w:t></w:r>''')
    runs_xml.append(f'''<w:r><w:rPr><w:rFonts w:ascii="TH Sarabun PSK" w:hAnsi="TH Sarabun PSK" w:cs="TH Sarabun PSK"/><w:sz w:val="24"/><w:szCs w:val="24"/><w:color w:val="333333"/></w:rPr><w:t xml:space="preserve">{esc(text)}</w:t></w:r>''')
    return f'<w:p>{p_pr}{"".join(runs_xml)}</w:p>'

def make_table(headers, rows, col_widths, alignments=None, font_size=22, header_bg="1E3A8A"):
    ...
    for row in rows:
        ...
        for i, val in enumerate(row):
            ...
            xml.append(f'''<w:tc><w:tcPr><w:tcW w:w="{w}" w:type="dxa"/><w:vAlign w:val="center"/></w:tcPr>
                <w:p><w:pPr><w:jc w:val="{al}"/><w:spacing w:before="20" w:after="20" w:line="200" w:lineRule="auto"/></w:pPr>
                <w:r><w:rPr><w:rFonts w:ascii="TH Sarabun PSK" w:hAnsi="TH Sarabun PSK" w:cs="TH Sarabun PSK"/><w:sz w:val="{font_size}"/><w:szCs w:val="{font_size}"/><w:color w:val="1E293B"/></w:rPr><w:t xml:space="preserve">{esc(val)}</w:t></w:r></w:p></w:tc>''')
        xml.append('</w:tr>')
    xml.append('</w:tbl>')
    return "".join(xml)
